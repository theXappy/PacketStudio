    ...
    public SctpDataChunk(ByteArraySegment bas)
    {
      ...
      if ((uint) this.Length % 4U > 0U)
      {
        ...
        if (num < NewSegmentLength + length)
          payload = new ByteArraySegment(((IEnumerable<byte>) this.Header.NextSegment(NewSegmentLength).ActualBytes()).Concat<byte>((IEnumerable<byte>) new byte[length]).ToArray<byte>())
          {
            Length = num
          };
        else
          payload = this.Header.NextSegment((int) this.Length);
      }
      else
        payload = this.Header.NextSegment((int) this.Length);
        this.PayloadPacketOrData = new LazySlim<PacketOrByteArraySegment>(() => SctpDataChunk.ParseEncapsulatedBytes(payload, this.PayloadProtocol));
    }
